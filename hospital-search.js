  ...
  async function searchArea(query) {
    ...
    try {
      ...
      if (cached?.data?.length) {
        hospitals = cached.data.map(normalize);
        updateLocationLabel(q);
        refresh();
        setStatus("⚠️ Live lookup is busy. Showing the last successful result for this area.");
      } else {
      ...
      async position => {
        ...
        try {
          ...
          if (cached?.data?.length) {
            ...
            updateLocationLabel("Your current location");
            refresh();
            setStatus("⚠️ Live lookup is busy. Showing the last successful nearby result.");
          } else {
            const message = friendly(error);
            problem("Live hospital lookup unavailable", message);
            setStatus(`⚠️ ${message}`);
